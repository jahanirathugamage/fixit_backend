...
async function deleteTimeBlocksForThisOccurrence(
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (role !== "provider") {
      return res.status(403).json({ error: "Only providers can cancel next job." });
    }
    ...
    // 4) Delete only the timeBlocks for THIS occurrence (best-effort)
    if (scheduledAt) {
    ...
    // You already support this route robustly in main.dart.
    if (clientId) {
